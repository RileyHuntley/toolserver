...
title = 'Deleted pages'
description = "This chart shows how many pages have been deleted in the last days."

projectdbs = getProjectDatabases()

queries = [
    ["Deletes", "SELECT CONCAT(YEAR(log_timestamp),'-',LPAD(MONTH(log_timestamp),2,'0'),'-',LPAD(DAY(log_timestamp),2,'0'),'T00:00:00Z') AS date, COUNT(*) AS count FROM logging WHERE log_timestamp>=DATE_ADD(NOW(), INTERVAL -%d DAY) AND log_action='delete' GROUP BY date ORDER BY date ASC" % (lastdays)],
]
...
js = """function p() {
    ...
    var data = [{ data: d1[selected], label: "Deletes"}];
