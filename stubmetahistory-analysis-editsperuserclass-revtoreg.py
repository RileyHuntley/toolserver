...
revto=re.compile(r'(?i)(posible|vandalismo|prueba|blanqueo|revertidos los cambios|deshecha la edición \d+|rev\. edición|rv\. edic\.) *[^\[]*? *(de|por) *\[\[(user\:|usuario\:|special\:contributions\/|especial\:contribuciones\/)(?P<reverteduser>[^\|\]]*?)[\|\]]')
for l in f:
    c+=1
    if c==1:
        continue
    ...
    if t:
        ...
        #reversion a usuario registrado?
        m=revto.finditer(comment)
        for i in m:
            reverteduser=i.group('reverteduser')
            if not re.search(ipregexp, reverteduser) and not bots.has_key(reverteduser):
                timestamps[timestamp2]['revtoreg']['edits']+=1
...
g.write('timestamp;admin;bot;reg;anon;all;revtoreg;\n')
antes=0
salida=u''
maxs=[]
total=admin=bot=reg=anon=revtoreg=0.0
for timestamp, userclasses in timestampslist:
    ...
    revtoreg+=userclasses['revtoreg']['edits']
    ...
    g.write('%s;%s;%s;%s;%s;%s;%s;\n' % (timestamp, userclasses['admin']['edits'], userclasses['bot']['edits'], userclasses['reg']['edits'], userclasses['anon']['edits'], userclasses['all']['edits'], userclasses['revtoreg']['edits']))
g.write('Total=%d;Admin=%d (%f%%);Bot=%d (%f%%);Reg=%d (%f%%);Anon=%d (%f%%);Revtoreg=%d;' % (total, admin, admin/(total/100), bot, bot/(total/100), reg, reg/(total/100), anon, anon/(total/100), revtoreg))
g.close()
