...
g.write('timestamp;admin;bot;reg;anon;all;\n')
antes=0
salida=u''
maxs=[]
total=admin=bot=reg=anon=0.0
...
g.write('Total=%d;Admin=%d (%f%%);Bot=%d (%f%%);Reg=%d (%f%%);Anon=%d (%f%%);' % (total, admin, admin/(total/100), bot, bot/(total/100), reg, reg/(total/100), anon, anon/(total/100)))
g.close()
