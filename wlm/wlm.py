...
"""
regexp['belarus']['be-x-old'] = re.compile(ur"""(?im)\{\{\s*Вікі любіць славутасьці/Элемэнт сьпісу\s*(\|\s*(назва\s*=\s*(?P<nombre>[^=}]*?)|населены пункт\s*=\s*(?P<municipio>[^=}]*?)|адрэса\s*=(?P<lugar>[^=}]*?)|шырата\s*=\s*(?P<lat>[0-9\.\-\+]*?)|даўгата\s*=\s*(?P<lon>[0-9\.\-\+]*?)|шыфр\s*=\s*(?P<bic>[^=}]*?)|выява\s*=\s*(?P<imagen>[^=}]*?)|датаваньне\s*=\s*([^=}]*?)|каардынаты\s*=\s*([^=}]*?)|катэгорыя\s*=\s*([^=}]*?)\s*)\s*)+\s*\|*\s*\}\}""")
